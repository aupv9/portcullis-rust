...
pub async fn run(cfg: Config, config_path: std::path::PathBuf) -> anyhow::Result<()> {
        ...
        tokio::sync::mpsc::channel(portcullis_provision::LIVENESS_BUFFER);
    {
        ...
        // Liveness poll cadence: from config (`liveness_poll_secs`, default 5 min), with the
        // PC_LIVENESS_POLL_SECS env var as a rebuild-free startup override. Floor-clamped so
        // a typo cannot hammer the MIPS shell-outs. A longer period only delays drift
        // detection and the post-apply "committed" re-confirm — never enforcement (the poll
        // is read-only).
        let liveness_secs = std::env::var("PC_LIVENESS_POLL_SECS")
            .ok()
            .and_then(|v| v.trim().parse::<u64>().ok())
            .unwrap_or(cfg.liveness_poll_secs)
            .max(portcullis_provision::MIN_POLL_INTERVAL_SECS);
        let liveness_interval = std::time::Duration::from_secs(liveness_secs);
        tracing::info!(interval_secs = liveness_secs, "wireless liveness poller cadence");
